...
Scene_Planes::Scene_Planes(GameEngine* gameEngine, const std::string& levelPath)
	: Scene(gameEngine), _worldView(gameEngine->window().getDefaultView()) {
	init(levelPath);
}

void Scene_Planes::init(const std::string& levelPath) {
	loadLevel(levelPath);
	registerActions();
	...
	// _worldBounds is the boundary of our game world.
	// this is a vertical scroller, the camera starts at the bottom of the game world bounds and scrolls up
...
void Scene_Planes::update(sf::Time dt) {
	sUpdate(dt);
}

void Scene_Planes::sDoAction(const Command& command) {
	// On Key Press
...
void Scene_Planes::drawAABB(std::shared_ptr<Entity> e) {
	if (_drawAABB) {
...
void Scene_Planes::drawCameraView() {
	if (_drawCam) {
...
void Scene_Planes::drawHP(sPtrEntt e) {
...
void Scene_Planes::drawAmmo(sPtrEntt e) {
...
void Scene_Planes::drawEntt(sPtrEntt e) {
	// Draw Sprite
...
void Scene_Planes::registerActions() {
...
void Scene_Planes::spawnPlayer(sf::Vector2f pos) {
...
void Scene_Planes::playerMovement() {
...
void Scene_Planes::annimatePlayer() {
...
void Scene_Planes::adjustPlayerPosition() {
...
void Scene_Planes::loadLevel(const std::string& path) {
...
void Scene_Planes::sMovement(sf::Time dt) {
	playerMovement();
	annimatePlayer();
...
void Scene_Planes::sUpdate(sf::Time dt) {
...
void Scene_Planes::onEnd() {
	_game->changeScene("MENU", nullptr, false);
}

void Scene_Planes::spawnMisille() {
...
void Scene_Planes::fireBullet() {
	if (_player->hasComponent<CGun>())
		_player->getComponent<CGun>().isFiring = true;
}

void Scene_Planes::sGunUpdate(sf::Time dt) {
	for (auto e : _entityManager.getEntities()) {
...
void Scene_Planes::spawnBullet(sf::Vector2f pos, bool isEnemy) {
	float speed;
	if (isEnemy) {
...
void Scene_Planes::sSpawnEnemies() {
...
void Scene_Planes::spawnEnemyPlanes(SpawnPoint sp) {
...
void Scene_Planes::sAutoPilot(sf::Time dt) {
...
sf::FloatRect Scene_Planes::getBattlefieldBounds() const {
	auto center = _worldView.getCenter();
...
void Scene_Planes::destroyOutsideBattlefieldBounds() {
...
sf::Vector2f Scene_Planes::findClosestEnemy(sf::Vector2f mPos) {
...
void Scene_Planes::sGuideMissiles(sf::Time dt) {
...
void Scene_Planes::checkPickupCollision() {
	for (auto e : _entityManager.getEntities("Pickup")) {
...
void Scene_Planes::checkPlaneCollision() {
	for (auto e : _entityManager.getEntities("enemy")) {
...
void Scene_Planes::checkBulletCollision() {
	// Player Bullets
...
void Scene_Planes::checkMissileCollision() {
...
void Scene_Planes::startAnimation(sPtrEntt e, std::string animation) {
}

void Scene_Planes::checkIfDead(sPtrEntt e) {
...
void Scene_Planes::dropPickup(sf::Vector2f pos) {
	// todo
...
void Scene_Planes::sAnimation(sf::Time dt) {
	for (auto e : _entityManager.getEntities()) {
